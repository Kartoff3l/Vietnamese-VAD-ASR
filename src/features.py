...
def compute_frame_rms(frame: np.ndarray) -> float:
    """Computes Root Mean Square (RMS) energy for a single audio frame."""
    if len(frame) == 0:
        return 0.0
...
def compute_frame_zcr(frame: np.ndarray) -> float:
    """Computes Zero Crossing Rate (ZCR) for a single audio frame."""
    if len(frame) <= 1:
        return 0.0
    signs = np.sign(frame)
    signs[signs == 0] = 1
...
def compute_rms_energy(
    ...
    """
    Computes RMS energy across 20ms frames.
    Returns (timestamps_s, rms_values).
    """
...
def compute_zcr(
    ...
    """
    Computes Zero Crossing Rate across 20ms frames.
    Returns (timestamps_s, zcr_values).
    """
    frames = frame_audio(audio, frame_duration_ms=frame_duration_ms, sample_rate=sample_rate)
